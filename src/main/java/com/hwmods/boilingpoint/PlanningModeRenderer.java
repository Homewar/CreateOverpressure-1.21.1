    ...
    static void onRenderLevel(RenderLevelStageEvent event) {
        ...
        for (PneumaticTubeBlockItem.PlanTube tube : result.tubes()) {
            if (tube.curveP0() == null) {
                drawGhostTube(poseStack, buffers.getBuffer(BoilingPointRenderTypes.ghostTube()), tube, routeColor);
                hasStraightTubes = true;
            }
        }
        if (hasStraightTubes) {
            buffers.endBatch(BoilingPointRenderTypes.ghostTube());
        }

        boolean hasCurveTubes = false;
        for (PneumaticTubeBlockItem.PlanTube tube : result.tubes()) {
            if (tube.curveP0() != null) {
                drawGhostTube(poseStack, buffers.getBuffer(BoilingPointRenderTypes.ghostCurveTube()), tube, routeColor);
                hasCurveTubes = true;
            }
        }
        if (hasCurveTubes) {
            buffers.endBatch(BoilingPointRenderTypes.ghostCurveTube());
        }
        poseStack.popPose();
    }
